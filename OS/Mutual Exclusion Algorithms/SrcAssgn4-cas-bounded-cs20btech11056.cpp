...
// an array to check if a particular thread is waiting to enter the critical section
vector<bool> waiting;
// atomic lock used in mutual exclusion
...
// log file
FILE *output = fopen("Bounded CAS-Log.txt", "w");
...
void testCS(int thread_id)
{
    ...
    for(i=0; i<number_of_repetitions; i++)
    {
        ...
        bool temp = false;
        bool key = true;
        waiting[thread_id-1] = true; // corresponding thread is waiting to enter critical section
        // storing the time just before it starts to wait for the lock
        begin_waiting_time = steady_clock::now();
        // trying to acquire the lock
        while(waiting[thread_id-1] && key == true)
        {
            key = !atomic_compare_exchange_strong(&lock, &temp, true);
            temp = false;
        }
        // loop breaks once lock is acquired and we store time at which it does so
        // critical section begins
        end_waiting_time = steady_clock::now();
        waiting[thread_id-1] = false; // since thread is now in critical section, it is no longer waiting
        ...
        int j = thread_id % number_of_threads;
        while((j != (thread_id-1)) && !waiting[j])
        {
            j = (j+1) % number_of_threads;
        }
...
int main() 
{
    ...
    ofstream stats_output("Bounded CAS-Stats.txt");
