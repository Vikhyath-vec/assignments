...
// log file
FILE *output = fopen("CAS-Log.txt", "w");
...
void testCS(int thread_id)
{
    ...
    for(i=0; i<number_of_repetitions; i++)
    {
        ...
        while(!atomic_compare_exchange_strong(&lock, &temp, true))
        {
            temp=false;
        }
...
int main() 
{
    ...
    {
        thread_ids[i] = i + 1;
        threads[i] = thread(testCS, thread_ids[i]);
    }
    ...
    ofstream stats_output("CAS-Stats.txt");
